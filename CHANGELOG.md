...
- **`playwright_rs::locator!()` compile-time-validated selector macro** — re-exported from the new companion crate `playwright-rs-macros` (independently versioned, starting at 0.1.0). Takes a string literal, rejects empty/whitespace-only input, unbalanced `[]`/`()`/`{}`, and unknown engine prefixes at compile time. Expands to the validated `&'static str`, so `page.locator(locator!("#submit"))` is identical to `page.locator("#submit")` at runtime. Brackets inside quoted attribute values are correctly skipped. Gated behind a default-on `macros` feature; opt out with `default-features = false` to skip the proc-macro dependency. This is the first piece of compile-time selector validation Rust can offer that JS, Python, Java, and .NET cannot — see the macros crate's own CHANGELOG and `tests/ui/README.md` for the workflow. Closes [#81](https://github.com/padamson/playwright-rust/issues/81).
