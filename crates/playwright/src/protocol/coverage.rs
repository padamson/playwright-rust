...
impl StartJSCoverageOptions {
    /// Whether to reset coverage on every navigation. Defaults to `true`.
    pub fn reset_on_navigation(mut self, reset_on_navigation: bool) -> Self {
        self.reset_on_navigation = Some(reset_on_navigation);
        self
    }

    /// Whether to report anonymous scripts generated by the page. Defaults
    /// to `false`.
    pub fn report_anonymous_scripts(mut self, report_anonymous_scripts: bool) -> Self {
        self.report_anonymous_scripts = Some(report_anonymous_scripts);
        self
    }
}
...
mod tests {
    ...
    #[test]
    fn js_coverage_setters_write_their_own_fields() {
        let opts = StartJSCoverageOptions::default()
            .reset_on_navigation(false)
            .report_anonymous_scripts(true);
        assert_eq!(opts.reset_on_navigation, Some(false));
        assert_eq!(opts.report_anonymous_scripts, Some(true));
    }

    #[test]
    fn css_coverage_setter_writes_the_field() {
        let opts = StartCSSCoverageOptions::default().reset_on_navigation(false);
        assert_eq!(opts.reset_on_navigation, Some(false));
    }
}
