...
- **`route.fulfill()` now delivers status, headers, and body to the browser, in all three engines.** The crate sent the parameters nested under a `response` key the driver ignored, and documented the result as a Playwright server limitation spanning 1.49.0 to 1.62.1. Header names are lowercased and `content-length` is filled from the body, matching playwright-python. HAR replay through `route_from_har` now carries several `set-cookie` headers the way upstream does, joined for the browser to split.
- **`route.continue_()` and `route.fallback()` no longer corrupt a string `post_data`.** The driver reads `postData` as base64; the string form was sent raw, so `post_data("key=value")` reached the server as two garbage bytes. Both forms are now encoded. The continue tests previously discarded their results, which is how this survived; they now assert what the server received.
