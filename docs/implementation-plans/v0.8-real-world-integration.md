...
- ✅ New public API: `Page::evaluate<T, U>()`
- ✅ Closes critical capability gap (argument passing to JavaScript)
- ✅ Significant user-facing feature addition
...
**Tasks:**
- [ ] **File Payload Improvements**: Add `FilePayload::from_path()` and `from_file()` with automatic MIME type guessing (crate `mime_guess`)
- [ ] **BrowserContext Options**: Add missing options:
...
- [ ] **Route API Review**: Verify `route.continue_()` overrides and `route.fulfill()` behavior (known limitation: body not sent)
- [ ] **Error Handling**: Add `Error::context()` to provide better failure location details (e.g. "Failed to click element")
